...
public class RealizaSaqueServiceImpl extends InsereOperacao implements RealizaSaqueService {
    ...
    public void process(Transacao transacao) {
        log.info(BF_DESAFIO2_00001I.getCodAndDescription() + CONST_DESCRICAO_SERVICE);
        ...
        log.info(BF_DESAFIO2_00002I.getCodAndDescription() + CONST_DESCRICAO_SERVICE);
    }

    private void aplicaConsistencias(Conta conta, double valorSaque, double limiteDisponivel) {
        if (valorSaque == 0) {
            throw new BusinessException(BF_DESAFIO2_00003E.getCodAndDescription());
        }

        if (conta.getSaldo() < valorSaque) {
            throw new BusinessException(BF_DESAFIO2_00004E.getCodAndDescription());
        }

        if (limiteDisponivel < valorSaque) {
            throw new BusinessException(BF_DESAFIO2_00005E.getCodAndDescription());
        }

        if (!conta.isFlagAtivo()) {
            throw new BusinessException(BF_DESAFIO2_00006E.getCodAndDescription());
        }
    }
}
